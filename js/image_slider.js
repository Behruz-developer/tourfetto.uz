...
// *** Global Text Rotation *** //
		$theCarouselWrapper.find('.home-carousel-rotate-wrapper').each(function(){
			...
			const $rotationDealy = typeof $theCarouselRotateWrapper.data('delay') !== 'undefined' ? parseInt($theCarouselRotateWrapper.data('delay')) : 0;
			setInterval(function () {
				// Set custom delay to stagger
				setTimeout(function(){
					let $currentLebel = $theCarouselRotateWrapper.find('.home-carousel-rotate-item:nth-of-type(1)');
					let $nextLebel = $theCarouselRotateWrapper.find('.home-carousel-rotate-item:nth-of-type(2)');	
...
// *** Background Image Rotation *** //
		setInterval(function () {
			...
			$theCarousel.find('.home-carousel-image').css('transition-duration',slideTransitionDuration);

			// Get all objects to manipulate
			let $currentPannel = $theCarousel.find('.home-carousel-panel:nth-of-type(1)');
			let $nextPannel = $theCarousel.find('.home-carousel-panel:nth-of-type(2)');	
			
			const $currentPannelImg1 = $currentPannel.find('.home-carousel-image:nth-of-type(1)');
			const $currentPannelImg2 = $currentPannel.find('.home-carousel-image:nth-of-type(2)');
			const $currentPannelImg3 = $currentPannel.find('.home-carousel-image:nth-of-type(3)');
			
			const $nextPannelImg1 = $nextPannel.find('.home-carousel-image:nth-of-type(1)');
			const $nextPannelImg2 = $nextPannel.find('.home-carousel-image:nth-of-type(2)');
			const $nextPannelImg3 = $nextPannel.find('.home-carousel-image:nth-of-type(3)');
			...
			setTimeout(function () {
			   ...
			   // Remove the class that disables the transition so future columns can slide
				setTimeout(function () {
					$theCarousel.find('.disableTransition').removeClass('disableTransition');
				}, slideTransitionDurationMS);
